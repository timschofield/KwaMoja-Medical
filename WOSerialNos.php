...
if (isset($_GET['Delete'])) { //user hit delete link
	...
	$result = DB_query("DELETE FROM woserialnos
						WHERE wo='" . $WO . "'
						AND stockid='" . $StockID . "'
						AND serialno='" . $_GET['Reference'] . "'", $db);
...
if ($Serialised == 1 and $NextSerialNo > 0) {
	...
	echo ':</td>
		<td><input type="text" name="NumberToAdd" size="10" class="number" minlength="0" maxlength="10" value="1" /></td>
	...
	if ($Serialised == 0) { //also need to add the quantity
		echo '<td><input type="text" name="Quantity" size="10" class="number" minlength="0" maxlength="10" value="1" /></td>';
