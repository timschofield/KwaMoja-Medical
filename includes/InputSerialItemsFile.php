...
$TotalQuantity = 0;
/*Variable to accumulate total quantity received */
$RowCounter = 0;
$k = 0;
...
if ((isset($_FILES['ImportFile']) and $_FILES['ImportFile']['error'] != '') and !isset($_SESSION['CurImportFile'])) {
	echo _('There was a problem with the uploaded file') . '. ' . _('We received') . ':<br />' . _('Name') . ':' . $_FILES['ImportFile']['name'] . '<br />' . _('Size') . ':' . locale_number_format($_FILES['ImportFile']['size'] / 1024, 2) . 'kb<br />' . _('Type') . ':' . $_FILES['ImportFile']['type'] . '<br />';
