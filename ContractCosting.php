...
if ($_SESSION['Contract' . $Identifier]->Status == 2) { //the contract is an order being processed now
	...
	echo '<form onSubmit="return VerifyForm(this);"  method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedContract=' . $_SESSION['Contract' . $Identifier]->ContractRef . '&amp;identifier=' . $Identifier . '">';
	echo '<div>';
