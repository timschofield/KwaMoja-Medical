...
if ($ReportSpec['groupbydata2'] != 'Not Used') {
	...
	$ReportSpec['groupbydata3'] = 'Not Used';
	/*This is forced if no entry in Group By 2 */
}
...
if ($ReportSpec['groupbydata3'] != 'Not Used') {
	...
	$ReportSpec['groupbydata4'] = 'Not Used';
	/*This is forced if no entry in Group By 3 */
...
while ($Cols = DB_fetch_array($ColsResult)) {
	if ($Cols['calculation'] == 1) {
		...
		} while (mb_strpos($SQLSelectCls, ',', $strt_ColNum) < $length_ColNum and mb_strpos($SQLSelectCls, ',', $strt_ColNum) != 0);
		...
		if (!($Cols['calcoperator'] == 'C' or $Cols['calcoperator'] == '*')) {
			...
			} while (mb_strpos($SQLSelectCls, ',', $strt_ColDen) < $length_ColDen and mb_strpos($SQLSelectCls, ',', $strt_ColDen) != 0);
		...
		} elseif ($Cols['calcoperator'] == 'C') {
			/* its a calculation divided by Constant */

			$SQLSelectCls = $SQLSelectCls . ', ' . mb_substr($SQLSelectCls, $strt_ColNum, $length_ColNum) . '/' . $Cols['constant'] . ' AS col' . ($Cols['colno'] + 8);

		} elseif ($Cols['calcoperator'] == '*') {
			/* its a calculation multiplied by constant */
			$SQLSelectCls = $SQLSelectCls . ', ' . mb_substr($SQLSelectCls, $strt_ColNum, $length_ColNum) . '*' . $Cols['constant'] . ' AS col' . ($Cols['colno'] + 8);
