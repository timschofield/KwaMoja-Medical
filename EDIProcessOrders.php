...
while (false !== ($OrderFile = readdir($dirhandle))) {
	/*there are files in the incoming orders dir */
	...
	$FirstSegInGroup = 0;
	$EmailText = '';
	/*Text of email to send to customer service person */
	$CreateOrder = True;
	/*Assume that we are to create a sales order in the system for the message read */

	$Order = new cart;

	while ($LineText = fgets($fp) and $TryNextFile != True) {
		/* get each line of the order file */
		...
		*/
		while ($SegTag != $Seg[$SegID]['SegTag'] and $SegID < $TotalNoOfSegments) {
			...
			$SegID++;
			/*Move to the next Seg in the order message */
			$LastSeg = $SegID;
			/*Remember the last segid moved to */
			...
			if ($Seg[$SegID]['SegGroup'] != $SegGroup and $Seg[$SegID]['MaxOccur'] > $SegCounter) {
				...
				$SegID = $FirstSegInGroup;
				/*Try going back to first seg in the group */
				if ($SegTag != $Seg[$SegID]['SegTag']) {
					/*still no match - must be into new seg group */
					$SegID = $LastSeg;
		...
		switch ($SegTag) {
			...
			case 'DTM':
				...
				switch ($DTM_C507[0]) {
					...
					/*dispatch date */
					case 'X14':
					/*Reguested delivery week commencing EAN code */
					case '64':
			...
			case 'NAD':
				...
				$NAD_C058 = explode(':', $NAD[2]);
				/*Not used according to MIG */
				$NAD_C080 = explode(':', $NAD[3]);
				$NAD_C059 = explode(':', $NAD[4]);
				switch ($NAD[0]) {
					case 'IV':
						/* This Name and address detail is that of the party to be invoiced */
