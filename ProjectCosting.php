...
$SQL = "SELECT stockmoves.stockid,
		...
		AND stockmoves.reference='" . $_SESSION['Project' . $Identifier]->WO . "'
		GROUP BY stockmoves.stockid,
				stockmaster.description,
				stockmaster.units,
				stockmaster.decimalplaces";
$ErrMsg = _('Could not get the inventory issues for this project because');
...
echo '<p class="page_title_text" >
			<img src="' . $RootPath . '/css/' . $_SESSION['Theme'] . '/images/project.png" title="' . _('Project') . '" alt="" />';
if ($_SESSION['Project' . $Identifier]->Status == 3) {
	echo _('Closed') . ' ';
...
echo _('Project') . '<br />' . $_SESSION['Project' . $Identifier]->DonorName . '<br />' . $_SESSION['Project' . $Identifier]->ProjectDescription . '</p>';
...
foreach ($_SESSION['Project' . $Identifier]->ProjectBOM as $Component) {
	...
	$ProjectBOMBudget += ($Component->ItemCost * $Component->Quantity);
...
foreach ($InventoryIssues as $Component) { //actual inventory components used
	$ProjectBOMActual -= $Component->TotalCost;
...
echo '<tr>
		...
		<td class="number">' . locale_number_format($ProjectBOMBudget, $_SESSION['CompanyRecord']['decimalplaces']) . '</td>
		<td class="number" colspan="5">' . _('Total Inventory Actual Cost') . ':</td>
		<td class="number">' . locale_number_format($ProjectBOMActual, $_SESSION['CompanyRecord']['decimalplaces']) . '</td>
	</tr>';
...
/*Now read in the actual other items charged to the project */
$SQL = "SELECT supptrans.supplierno,
				supptrans.suppreference,
				supptrans.trandate,
				projectcharges.amount,
				projectcharges.narrative,
				projectcharges.anticipated
		FROM supptrans INNER JOIN projectcharges
		ON supptrans.type=projectcharges.transtype
		AND supptrans.transno=projectcharges.transno
		WHERE projectcharges.projectref='" . $ProjectRef . "'
		ORDER BY projectcharges.anticipated";
$ErrMsg = _('Could not get the other charges to the project because');
...
echo '<tr>
		<td colspan="5"><b>' . _('Total Budget Project Cost') . '</b></td>
		<td class="number"><b>' . locale_number_format($OtherReqtsBudget + $ProjectBOMBudget, $_SESSION['CompanyRecord']['decimalplaces']) . '</b></td>
		<td colspan="5"><b>' . _('Total Actual Project Cost') . '</b></td>
		<td class="number"><b>' . locale_number_format($OtherReqtsActual + $ProjectBOMActual, $_SESSION['CompanyRecord']['decimalplaces']) . '</b></td>
	</tr>';
...
if (isset($_POST['CloseProject']) and $_SESSION['Project' . $Identifier]->Status == 2) {
	...
	$Variance = ($OtherReqtsBudget + $ProjectBOMBudget) - ($OtherReqtsActual + $ProjectBOMActual);

	$ProjectCloseNo = GetNextTransNo(32);
	...
	$SQL = "INSERT INTO gltrans (type,
					...
					VALUES ( 32,
							...
							'" . $PeriodNo . "',
							'" . $GLCodes['wipact'] . "',
							'" . _('Variance on project') . ' ' . $_SESSION['Project' . $Identifier]->ProjectRef . "',
							'" . -$Variance . "')";

	$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The gl entry of WIP for the variance on closing the project could not be inserted because');
	...
	//Now update the status of the project to closed
	$SQL = "UPDATE projects
				SET status=3
				WHERE projectref='" . $_SESSION['Project' . $Identifier]->ProjectRef . "'";
	$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The status of the project could not be updated to closed because');
	$DbgMsg = _('The following SQL to change the status of the project was used');
	$Result = DB_query($SQL, $ErrMsg, $DbgMsg, true);

	/*Check if the project work order is still open */
	$CheckIfWOOpenResult = DB_query("SELECT closed
									FROM workorders
									WHERE wo='" . $_SESSION['Project' . $Identifier]->WO . "'");
	$CheckWORow = DB_fetch_row($CheckIfWOOpenResult);
	if ($CheckWORow[0] == 0) {
		//then close the work order
		$CloseWOResult = DB_query("UPDATE workorders
									SET closed=1
									WHERE wo='" . $_SESSION['Project' . $Identifier]->WO . "'", _('Could not update the work order to closed because') . ': ', _('The SQL used to close the work order was') . ': ', true);


		/* Check if the project BOM has received the project item manually
		 * If not then process this as by closing the project the user is saying it is complete
		 *  If work done on the project is a write off then the user must also write off the stock of the project item as a separate job
		 */

		$Result = DB_query("SELECT qtyrecd FROM woitems
							WHERE stockid='" . $_SESSION['Project' . $Identifier]->ProjectRef . "'
							AND wo='" . $_SESSION['Project' . $Identifier]->WO . "'");
		if (DB_num_rows($Result) == 1) {
			$MyRow = DB_fetch_row($Result);
			if ($MyRow[0] == 0) { //then the project wo has not been received (it will only ever be for 1 item)
				...
				$SQL = "SELECT locstock.quantity
						FROM locstock
						WHERE locstock.stockid='" . $_SESSION['Project' . $Identifier]->ProjectRef . "'
						AND loccode= '" . $_SESSION['Project' . $Identifier]->LocCode . "'";
				...
				$SQL = "INSERT INTO stockmoves (stockid,
							...
							VALUES ('" . $_SESSION['Project' . $Identifier]->ProjectRef . "',
									...
									CURRENT_DATE,
									'" . $_SESSION['UserID'] . "',
									'" . ($OtherReqtsBudget + $ProjectBOMBudget) . "',
									'" . $PeriodNo . "',
									'" . $_SESSION['Project' . $Identifier]->WO . "',
									1,
									'" . ($OtherReqtsBudget + $ProjectBOMBudget) . "',
									'" . ($QtyOnHandPrior + 1) . "')";
				...
				if ($_SESSION['CompanyRecord']['gllink_stock'] == 1 and ($OtherReqtsBudget + $ProjectBOMBudget) != 0) {
					...
					$SQL = "INSERT INTO gltrans (type,
									...
									VALUES (26,
											...
											'" . $PeriodNo . "',
											'" . $GLCodes['stockact'] . "',
											'" . $_SESSION['Project' . $Identifier]->WO . ' ' . $_SESSION['Project' . $Identifier]->ProjectRef . ' -  x 1 @ ' . locale_number_format(($OtherReqtsBudget + $ProjectBOMBudget), 2) . "',
											'" . ($OtherReqtsBudget + $ProjectBOMBudget) . "')";

					$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The receipt of project work order finished stock GL posting could not be inserted because');
				...
				$UpdateWOResult = DB_query("UPDATE woitems
										...
										WHERE wo='" . $_SESSION['Project' . $Identifier]->WO . "'
										AND stockid='" . $_SESSION['Project' . $Identifier]->ProjectRef . "'", $ErrMsg, $DbgMsg, true);
			} //end if the project wo was not received - work order item received/processed above if not
	...
	$_SESSION['Project' . $Identifier]->Status = 3;
	prnMsg(_('The project has been closed. No further charges can be posted against this project.'), 'success');

} //end if Closing the project Close Project button hit
