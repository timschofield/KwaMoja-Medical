...
if (isset($_GET['Close'])) {
	...
	echo '<div class="centre">
			<form onSubmit="return VerifyForm(this);" method="post" class="noPrint" action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedShipment=' . $_GET['SelectedShipment'] . '">';
	...
	if ($_SESSION['WeightedAverageCosting'] == 0) {
		...
		echo _('Update Standard Costs') . ':<select required="required" minlength="1" name="UpdateCost">
...
if (isset($_POST['Close'])) {
	...
	$result = DB_query("UPDATE purchorderdetails
								   SET quantityord=quantityrecd,
									   completed=1
							WHERE shiptref = '" . $_GET['SelectedShipment'] . "'", $db, _('Could not complete the purchase order lines on this shipment'), '', TRUE);
