...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $_SESSION['Theme'] . '/images/maintenance.png" title="' . _('Geocode Setup') . '" alt="" />' . ' ' . _('Geocoding of Customers and Suppliers') . '</p>';
...
while ($row2 = DB_fetch_array($Result2)) {
	...
	while ($geocode_pending) {
		...
		if (strcmp($status, "OK") == 0) {
			...
			$query = sprintf("UPDATE suppliers " . " SET lat = '%s', lng = '%s' " . " WHERE supplierid = '%s' LIMIT 1;", ($lat), ($lng), ($id));
