...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedArea)) {
		...
		echo '<table class="selection">
			<tr>
				<td>' . _('Area Code') . ':</td>
				<td><input tabindex="1" ' . (in_array('AreaCode', $Errors) ? 'class="inputerror"' : '') . ' type="text" name="AreaCode" value="' . $_POST['AreaCode'] . '" size="3" minlength="0" maxlength="3" /></td>
			</tr>';
	}

	echo '<tr><td>' . _('Area Name') . ':</td>
		<td><input tabindex="2" ' . (in_array('AreaDescription', $Errors) ? 'class="inputerror"' : '') . '  type="text" name="AreaDescription" value="' . $_POST['AreaDescription'] . '" size="26" minlength="0" maxlength="25" /></td>
		</tr>';
