...
if (!isset($_GET['action']) or (!isset($_POST['ReportID']))) {
	...
	for ($i = 1; $i < 1000; $i++) { // figure out what sequence button was pressed
		if (isset($_POST['up' . $i . '_x']) or isset($_POST['dn' . $i . '_x'])) {
			$SeqNum = $i;
			break;
		}
	}
	...
	switch ($_POST['todo']) {
		...
		case RPT_BTN_CPYRPT:
			...
			if ($Prefs['defaultreport'])
				$Prefs['reportname'] = ''; // clear name if default report
			$ShowReplace = false;
			$Title = RPT_PAGESAVE;
			$IncludePage = 'forms/ReportsSave.html';
			break;
		...
		case RPT_BTN_EXPPDF:
			...
			if ($success['level'] == 'success') { // Generate the output data array
				...
				if (!$ReportData)
					$usrMsg[] = array(
						'message' => RPT_NODATA . ' The failing sql=' . $SQL,
						'level' => 'warn'
					);
...
function BuildCriteria($FieldListings) {
	...
	switch (array_shift($CritBlocks)) { // determine how many text boxes to build
		default:
		case 0:
			$EndString = '<td>&nbsp;</td><td>&nbsp;</td>';
			break;
		case 1:
			$EndString = '<td><input name="fromvalue' . $SeqNum . '" type="text"
				value="' . $Params[1] . '" size="21" maxlength="20"></td><td>&nbsp;</td>';
			break;
		case 2:
			$EndString = '<td><input name="fromvalue' . $SeqNum . '" type="text" value="' . $Params[1] . '" size="21" maxlength="20"></td>
