...
if (isset($Patient)) {
	...
	echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $_SESSION['Theme'] . '/images/XRay.png" title="' . _('Search') . '" alt="" />' . $Title . '</p>';
	...
	for ($i = 0; $i < $_SESSION['Items']['Lines']; $i++) {
		if (isset($_SESSION['Items'][$i])) {
			...
			echo '<td>&nbsp;@&nbsp;' . locale_number_format($_SESSION['Items'][$i]['Price'], $DecimalPlaces) . ' ' . $_SESSION['CompanyRecord']['currencydefault'] . '</td>';
	...
	echo '<td>' . locale_number_format($_SESSION['Items']['Value'], $DecimalPlaces) . ' ' . $_SESSION['CompanyRecord']['currencydefault'] . '</td></tr>';
	...
	if ($Patient[1] == 'CASH') {
		...
		echo '<td><input type="text" class="number" size="10" name="Received" value="' . locale_number_format($Received, $DecimalPlaces) . '" /></td></tr>';
