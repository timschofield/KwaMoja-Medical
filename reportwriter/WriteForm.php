...
class PDF extends PDF_Language {
	...
	function FormLine($Params) {
		if (!isset($Params['LineXStrt']))
			return; // don't do anything if data array has not been set
		if ($Params['Line'] == '2')
			$RGB = $Params['BrdrRed'] . ':' . $Params['BrdrGreen'] . ':' . $Params['BrdrBlue'];
		else
			$RGB = $Params['BrdrColor'];
		$FC = explode(':', $RGB);
	...
	function FormRect($Params) {
		...
		if ($Params['Fill'] == '0') { // Set Fill Color
			...
			$DrawFill .= 'F';
			if ($Params['Fill'] == '2')
				$RGB = $Params['FillRed'] . ':' . $Params['FillGreen'] . ':' . $Params['FillBlue'];
			else
				$RGB = $Params['FillColor'];
			$FC = explode(':', $RGB);
	...
	function FormText($Params) {
		...
		if ($Params['Color'] == '2')
			$RGB = $Params['FontRed'] . ':' . $Params['FontGreen'] . ':' . $Params['FontBlue'];
		else
			$RGB = $Params['FontColor'];
		$FC = explode(':', $RGB);
		...
		if ($Params['index'] <> 'PgNum')
			$TextField = $Params['TextField'];
		else
			$TextField = $this->PageNo() - $this->PageCnt;
		if (isset($Params['Processing']))
			$TextField = $this->ProcessData($TextField, $Params['Processing']);
	...
	function FormTable($Params) {
		...
		foreach ($Params['Data'] as $index => $MyRow) {
			...
			if ($FillThisRow and $Params['Fill'])
				$this->SetFillColor($FC[0], $FC[1], $FC[2]);
			else
				$this->SetFillColor(255);
			...
			// fill in the data
			if ($index == 0)
				$MaxRowHt = $this->ShowTableRow($Params, $MyRow, $Heading = true);
			else
				$MaxRowHt = $this->ShowTableRow($Params, $MyRow, $Heading = false);
	...
	function ShowTableRow($Params, $MyRow, $Heading) {
			...
			if ($Params['Seq'][$Col]['TblShow']) {
				...
				if ($Heading) { // center the heading
					...
					if (isset($Params['Seq'][$Col]['Processing']))
						$Value = $this->ProcessData($Value, $Params['Seq'][$Col]['Processing']);
				}
		...
		if ($ThisRowHt > $MaxRowHt)
			$MaxRowHt = $ThisRowHt; // keep that largest row so far to track pagination
	...
	function DrawTableLines($Params, $MaxBoxY) {
		...
		if ($Params['Line'] == '0')
			return; // return if draw lines is turned off
	...
	function ProcessData($strData, $Process) {
		switch ($Process) {
			case "uc":
				return mb_strtoupper($strData);
			case "lc":
				return strtolower($strData);
			case "neg":
				return -$strData;
			case "rnd2d":
				return locale_number_format($strData, $_SESSION['CompanyRecord']['decimalplaces']);
			case "dlr":
				return '$ ' . locale_number_format($strData, $_SESSION['CompanyRecord']['decimalplaces']);
			case "euro":
				return chr(128) . ' ' . locale_number_format($strData, $_SESSION['CompanyRecord']['decimalplaces']); // assumes standard FPDF fonts
			default:
				return $strData; // do nothing if Process not recognized
		}
	}
...
function BuildPDF($ReportID, $Prefs) {
	...
	// Fetch the Criteria
	$strCrit = '';
	if (is_array($Prefs['CritListings']))
		while ($FieldValues = array_shift($Prefs['CritListings'])) {
			$Params = explode(':', $FieldValues['params']);
			switch ($Params[1]) {
				case RPT_RANGE:
					if (mb_strlen($strCrit) > 0) {
						$strCrit .= ' AND ';
					}
					$t = '';
	...
	if ($Prefs['table2'])
		$SQLTable .= ' INNER JOIN ' . $Prefs['table2'] . ' ON ' . $Prefs['table2criteria'];
	if ($Prefs['table3'])
		$SQLTable .= ' INNER JOIN ' . $Prefs['table3'] . ' ON ' . $Prefs['table3criteria'];
	if ($Prefs['table4'])
		$SQLTable .= ' INNER JOIN ' . $Prefs['table4'] . ' ON ' . $Prefs['table4criteria'];
	if ($Prefs['table5'])
		$SQLTable .= ' INNER JOIN ' . $Prefs['table5'] . ' ON ' . $Prefs['table5criteria'];
	if ($Prefs['table6'])
		$SQLTable .= ' INNER JOIN ' . $Prefs['table6'] . ' ON ' . $Prefs['table6criteria'];

	// Build query string and execute
	$SQLCrit = '';
	if ($strCrit and $strDate) {
	...
	foreach ($FieldListings as $Key => $SingleObject) {
		if ($SingleObject['params']['index'] == 'CBlk') {
			...
			$TextField = '';
			foreach ($TxtBlkValues as $t => $Temp)
				$TextField .= $Temp . AddSep($SingleObject['params']['Seq'][$t]['Processing']);
	...
	foreach ($FormPageID as $formNum => $Fvalue) {
		...
		if ($SQLCrit) {
			$TrailingSQL = " FROM " . $SQLTable . " WHERE " . $SQLCrit . " AND " . $PageBreakField . "='" . $Fvalue . "'";
		} else {
			$TrailingSQL = " FROM " . $SQLTable . " WHERE " . $PageBreakField . "='" . $Fvalue . "'";
		}
