...
if (!isset($_POST['Search']) and (isset($_POST['Select']) or isset($_SESSION['SelectedStockItem']))) {
	...
	if (!$Its_A_Kitset) {
		...
		if (isset($_SESSION['CustomerID']) and $_SESSION['CustomerID'] != '' and mb_strlen($_SESSION['CustomerID']) > 0) {
			echo '<a href="' . $RootPath . '/Prices_Customer.php?Item=' . $StockID . '">' . _('Special Prices for customer') . ' - ' . stripslashes($_SESSION['CustomerID']) . '</a>';
		}
		...
		echo '<a href="' . $RootPath . '/PriceMatrix.php?StockID=' . $StockID . '">' . _('Mantain prices by quantity break and sales types') . '</a>';
	}
	echo '</td></tr></table>';
} else {
...
if (isset($_POST['Keywords'])) {
	echo '<input type="search" name="Keywords" autofocus="autofocus" value="' . $_POST['Keywords'] . '" size="20" minlength="0" maxlength="25" />';
} else {
	echo '<input type="search" name="Keywords" autofocus="autofocus" size="20" minlength="0" maxlength="25" />';
}
echo '</td></tr><tr><td></td>';
echo '<td><b>' . _('OR') . ' ' . '</b>' . _('Enter partial') . ' <b>' . _('Stock Code') . '</b>:</td>';
echo '<td>';
if (isset($_POST['StockCode'])) {
	echo '<input type="search" name="StockCode" value="' . $_POST['StockCode'] . '" size="15" minlength="0" maxlength="18" />';
} else {
	echo '<input type="search" name="StockCode" size="15" minlength="0" maxlength="18" />';
}
echo '</td></tr></table>';
