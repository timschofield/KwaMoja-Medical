...
if (isset($_GET['GRNNo']) and isset($_POST['SupplierID'])) {
	...
	if (DB_num_rows($Result) == 1) {
		/* if the GRN is in fact a stock item being reversed */
	...
	if ($_SESSION['CompanyRecord']['gllink_stock'] == 1 and $GRN['glcode'] != 0 and $GRN['stdcostunit'] != 0) {
		...
		$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (
								...
								'" . $GRN['deliverydate'] . "',
								'" . $PeriodNo . "',
								'" . $_SESSION['CompanyRecord']['grnact'] . "', '" . _('GRN Reversal PO') . ': ' . $GRN['orderno'] . " " . $_POST['SupplierID'] . " - " . $GRN['itemcode'] . "-" . $GRN['itemdescription'] . " x " . $QtyToReverse . " @ " . locale_number_format($GRN['stdcostunit'], $_SESSION['CompanyRecord']['decimalplaces']) . "',
								'" . $GRN['stdcostunit'] * $QtyToReverse . "'
								)";
	...
	echo '<td>' . _('Show all goods received after') . ': </td>
			<td><input type="text" class="date" alt="' . $_SESSION['DefaultDateFormat'] . '" name="RecdAfterDate" value="' . $_POST['RecdAfterDate'] . '" minlength="0" maxlength="10" size="10" /></td>
		</tr>
		</table>
		<br />
		<div class="centre">
