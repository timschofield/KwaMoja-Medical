...
echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/maintenance.png" title="' . _('Geocode Setup') . '" alt="" />' . ' ' . _('Geocoding of Customers and Suppliers') . '</p>';
...
while ($row = @mysql_fetch_assoc($result)) {
	...
	while ($geocode_pending) {
		...
		if (strcmp($status, "200") == 0) {
			...
			$query = sprintf("UPDATE custbranch " . " SET lat = '%s', lng = '%s' " . " WHERE branchcode = '%s' " . " AND debtorno = '%s' LIMIT 1;", mysql_real_escape_string($lat), mysql_real_escape_string($lng), mysql_real_escape_string($id), mysql_real_escape_string($debtorno));
...
while ($row2 = @mysql_fetch_assoc($result2)) {
	...
	while ($geocode_pending) {
		...
		if (strcmp($status, "200") == 0) {
			...
			$query = sprintf("UPDATE suppliers " . " SET lat = '%s', lng = '%s' " . " WHERE supplierid = '%s' LIMIT 1;", mysql_real_escape_string($lat), mysql_real_escape_string($lng), mysql_real_escape_string($id));
			$update_result = mysql_query($query);
			if (!$update_result) {
				die("Invalid query: " . mysql_error());
			}
