	...
	for ($i = 0; $i < $loop_max; $i++) {
		if (isset($_POST['SerialNo' . $i]) and mb_strlen($_POST['SerialNo' . $i]) > 0) {
			...
			if ($ExistingBundleQty > 0) {
				...
				if ($AddThisBundle == true AND filter_number_format($_POST['Qty' . $i]) > 0) {

					$LineItem->SerialItems[$_POST['SerialNo' . $i]] = new SerialItem($_POST['SerialNo' . $i], filter_number_format($_POST['Qty' . $i]), $_POST['ExpiryDate' . $i]);
				}
			}
