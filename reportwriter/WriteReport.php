	...
	function Header() {
		...
		foreach ($Heading as $Key => $Value) {
			...
			if ($ColBreak[$Key]) {
				...
				$LastY = $this->GetY();
			if ($this->GetY() > $maxY)
				$maxY = $this->GetY(); // check for new col max height
		}
...
function BuildSQL($Prefs) {
	...
	if (is_array($Prefs['FieldListings']))
		while ($FieldValues = array_shift($Prefs['FieldListings'])) {
			if ($FieldValues['visible'])
				$strField .= $FieldValues['fieldname'] . ', ';
		}
	...
	$strGroup = '';
	if (is_array($Prefs['GroupListings']))
		while ($FieldValues = array_shift($Prefs['GroupListings'])) {
	...
	switch ($DateArray[0]) { // based on the date choice selected
		...
		case "h": // RPT_GROUP_QUARTER
			...
			if ($QtrEnd == '04' or $QtrEnd == '06' or $QtrEnd == '09' or $QtrEnd == '11') {
				$TotalDays = '30';
			}
	...
	$strDate = $d;
	if ($fildesc <> '')
		$Prefs['filterdesc'] .= $fildesc; // update the filter description string
	...
	if ($filCrit <> '')
		$Prefs['filterdesc'] .= ' ' . RPT_CRITBY . ' ' . $filCrit . ';';
...
function BuildDataArray($ReportID, $SQL, $Prefs) {
	...
	$GrpFieldName = '';
	if (is_array($Prefs['GroupListings']))
		while ($Temp = array_shift($Prefs['GroupListings'])) {
			if ($Temp['params'] == '1')
	...
	if ($GrpWorking) { // if we collected group data show the final group total
		...
		foreach ($Seq as $TotalCtl) {
			if ($TotalCtl['total'] == '1')
				$OutputArray[$RowCnt][$ColCnt] = $TotalCtl['grptotal'];
			else
...
function GenerateCSVFile($Data, $Prefs) {
	...
	foreach ($Data as $MyRow) {
		...
		switch ($todo[0]) {
			...
			case "g": // Group Total
				if ($todo[0] == 'g')
					$Desc = 'Group Total for: ';
				else
					$Desc = 'Report Total for: ';
